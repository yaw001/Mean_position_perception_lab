function stimuli(trial_number) {
    ...
    console.log(coord)
    if (trial_index[trial_number][0] == 1) {
...
function draw(group_1_coord, group_2_coord, radius, color) {
    draw_group_1(group_1_coord, radius, color);
    if (trial_index[trialNumber][1] == 1) {
        draw_group_2(group_2_coord, radius, color);
    } 
}


function draw_group_1(group_1_coord, radius, color) {
    group_1_coord.map(x => circle(x.x, x.y, radius, color));
}

function draw_group_2(group_2_coord, radius, color) {
...
getClickPosition = function (e) {
    ...
    if (on) {
        ...
        // draw(group_1_coord, group_2_coord, radius, color);
...
function trialDone() {
    trialData.push({
        ...
        group_dist: trial_index[trialNumber][3],
...
function trial() {
    stimuli(trialNumber);
    setTimeout(() => { draw(group_1_coord, group_2_coord, radius, color, trialNumber); }, 500);
