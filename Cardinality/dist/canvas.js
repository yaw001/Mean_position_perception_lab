...
// Practice trial
var practice_1 = [1, 1, 1, -2];
var practice_2 = [2, 4, 1, -3];
var practice_3 = [2, 8, 1, -4];
var practice_4 = [4, 32, 1, -5];
var practice_5 = [8, 16, 1, -6];
var practice_6 = [8, 8, 1, -7];

// Trial structure: 6 practice trials + 18*36 = 648 (experimental trials) + 6 attention-check trials = 660 trials
var trial_size_pairs = repeat_whole_single(size_pairs, 36);
...
function get_positions(trial_index) {
    ...
    if (trial_index[3] == -1) {
        ...
        if (trial_index[0] == 1 && trial_index[1] == 1) {
            group_1_coord.x = trial_index[4] + group_mean_to_center * Math.cos(degToRad(180 + trial_index[3] * 30));
            group_1_coord.y = trial_index[5] + group_mean_to_center * Math.sin(degToRad(180 + trial_index[3] * 30));

            group_2_coord.x = trial_index[4] + group_mean_to_center * Math.cos(degToRad(trial_index[3] * 30));
            group_2_coord.y = trial_index[5] + group_mean_to_center * Math.sin(degToRad(trial_index[3] * 30));
            all_coord = [group_1_coord].concat([group_2_coord]);
        } else if (trial_index[0] != 1 && trial_index[1] == 1) {
            group_1_coord.map(x => x.x = (trial_index[4] + group_mean_to_center * Math.cos(degToRad(180 + trial_index[3] * 30))) + x.x * trial_index[2] * (group_range_unit));
            group_1_coord.map(x => x.y = trial_index[5] + group_mean_to_center * Math.sin(degToRad(180 + trial_index[3] * 30)) + x.y * trial_index[2] * (group_range_unit));
            ...
            group_2_coord.map(x => x.x = (trial_index[4] + group_mean_to_center * Math.cos(degToRad(trial_index[3] * 30))) + x.x * trial_index[2] * (group_range_unit));
            group_2_coord.map(x => x.y = trial_index[5] + group_mean_to_center * Math.sin(degToRad(trial_index[3] * 30)) + x.y * trial_index[2] * (group_range_unit));
